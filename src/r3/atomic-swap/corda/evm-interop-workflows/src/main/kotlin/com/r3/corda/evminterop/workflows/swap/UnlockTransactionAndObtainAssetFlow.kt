...
//    override val progressTracker: ProgressTracker = tracker()
    ...
    override fun call(): SignedTransaction {
        ...
        val oldOwner = lockedAsset.state.data.owner.owningKey
        if(oldOwner is CompositeKey) {
            serviceHub.registerCompositeKey(ourIdentity, lockedAsset.state.data.owner.owningKey)
        }
        ...
        val participantsSessions = (lockedAsset.state.data.participants - ourIdentity).map {
            //if(it.owningKey
            initiateFlow(it).also {
                it.send(oldOwner)
            }
        }
...
class UnlockTransactionAndObtainAssetFlowResponder(val session: FlowSession) : FlowLogic<Unit>() {
    ...
    @Suspendable
    override fun call() {
        session.receive<PublicKey>().unwrap {
            if(it is CompositeKey) {
                serviceHub.registerCompositeKey(ourIdentity, it)
            }
        }
