...
library ED25519 {
  ...
  /*
   * Verification of an EDDSA-ED25519-SHA512 signature.
   * @param {bytes32} k The compressed EDDSA public key to be used in verification.
   * @param {bytes32} r The r value of the EDDSA signature.
   * @param {bytes32} s The s value of the EDDSA signature.
   * @param {bytes} m The SHA512 hashed message that was signed.
  ...
  /*
   * Helper function to compute (v^(2^250-1), v^11) mod p.
   * @param {uint256} v The input parameter.
   * @return {uint256} p22501 Returns v^(2^250-1) mod p.
   * @return {uint256} p11 Returns v^11 mod p.
   */
  function pow22501(
    uint256 v
  ) internal pure returns (uint256 p22501, uint256 p11) {
