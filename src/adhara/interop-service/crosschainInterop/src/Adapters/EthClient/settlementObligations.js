...
function init(config, dependencies){
  ...
  async function createAndMakeHoldPerpetual(chainName, tradeId, fromAccountId, toAccountId, amount, localOperationId) {
    let createHoldResponse = await assetTokenContract.createHold(chainName, localOperationId, fromAccountId, toAccountId, amount * Math.pow(10, decimals))
    logger.log('debug', 'Response from asset token contract create hold: '+ JSON.stringify(createHoldResponse, null, 2))
    let makeHoldPerpetualResponse = await assetTokenContract.makeHoldPerpetual(chainName, localOperationId)
